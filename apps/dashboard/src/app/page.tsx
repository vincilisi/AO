...
export default function Dashboard() {
  ...
  async function saveOnboarding(event: FormEvent<HTMLFormElement>) {
    event.preventDefault();
    const data = Object.fromEntries(new FormData(event.currentTarget));
    const response = await apiFetch("/api/onboarding", { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ tone: data.tone, signature: data.signature, instructions: data.instructions, autoReplyEnabled: data.autoReplyEnabled === "on", confidenceThreshold: Number(data.confidenceThreshold), modules: onboarding?.onboarding.modules }) });
    const result = await response.json();
      ...
      <section className="workspace">
        ...
        {!loading && view === "overview" && overview && (
          <div className="view-content reveal">
            {onboarding && !onboarding.onboarding.completed && <section className="onboarding-panel"><div><p className="eyebrow">Avvio guidato</p><h2>Rendi operativo il tuo ufficio AI</h2><div className="onboarding-steps">{onboarding.steps.map((step, index) => <span className={step.completed ? "done" : ""} key={step.id}><b>{step.completed ? "✓" : index + 1}</b>{step.label}</span>)}</div></div><form onSubmit={saveOnboarding}><label>Tono<select name="tone" defaultValue={onboarding.aiConfiguration.tone}><option value="professionale">Professionale</option><option value="cordiale">Cordiale</option><option value="diretto">Diretto</option></select></label><label>Firma<input name="signature" defaultValue={onboarding.aiConfiguration.signature ?? account?.company.name ?? ""} /></label><label>Istruzioni<textarea name="instructions" defaultValue={onboarding.aiConfiguration.instructions ?? "Rispondi con chiarezza e non inventare prezzi."} /></label><label className="check-label"><input name="autoReplyEnabled" type="checkbox" defaultChecked={onboarding.aiConfiguration.autoReplyEnabled} /> Risposte automatiche</label><input name="confidenceThreshold" type="hidden" value="0.85" /><button className="primary-button" type="submit">{onboarding.mailboxConnected ? "Completa onboarding" : "Salva e collega email"}</button></form></section>}
