...
import {
  Activity, BriefcaseBusiness, Building2, CheckCircle2, CircleDollarSign, Clock3, Download,
  Inbox, LayoutDashboard, LogOut, Mail, Menu, Plus, RefreshCw, Search, Send, Settings, Sparkles, Users, X
} from "lucide-react";
...
interface Account { user: { name: string; email: string; role: string }; company: { name: string; email: string; vatNumber: string | null; phone: string | null; address: string | null; city: string | null; postalCode: string | null } }
...
export default function Dashboard() {
  ...
  async function loadData() {
    ...
    try {
      const responses = await Promise.all([
        ...
        apiFetch("/api/quotes"), apiFetch("/api/employees"), apiFetch("/api/system/status"), apiFetch("/api/auth/me"), apiFetch("/api/mailboxes"), apiFetch("/api/emails"), apiFetch("/api/products"), apiFetch("/api/plans"), apiFetch("/api/subscription")
      ]);
  ...
  async function createQuote(event: FormEvent<HTMLFormElement>) {
    ...
    const data = new FormData(event.currentTarget);
    try {
      const response = await apiFetch("/api/quotes", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ customerId: data.get("customerId"), title: data.get("title"), taxRate: Number(data.get("taxRate")), validUntil: data.get("validUntil"), notes: data.get("notes"), items: [{ productId: data.get("productId") || undefined, description: data.get("description"), quantity: Number(data.get("quantity")), unitPrice: Number(data.get("unitPrice")) }] }) });
      const result = await response.json();
  ...
  return (
    <main className="app-shell">
      <aside className={`sidebar ${mobileMenu ? "sidebar-open" : ""}`}>
        <div className="brand"><span className="brand-mark">AO</span><div><strong>AI Office</strong><small>Manager</small></div></div>
      ...
      <section className="workspace">
        ...
        {!loading && view === "plans" && <div className="view-content reveal">
          <PageIntro title="Scegli il piano SaaS" text="Tre configurazioni dimensionate sul volume di lavoro. Canone e installazione restano sempre separati e trasparenti." />
          ...
          <section className="pricing-grid">{plans.map((plan) => { const selected = subscription?.planCode === plan.code; const paypalActive = Boolean(subscription?.paypalSubscriptionId) && ["ACTIVE", "TRIALING", "PAST_DUE", "SUSPENDED"].includes(subscription?.status ?? ""); return <article className={`pricing-card ${plan.highlighted ? "featured" : ""} ${selected ? "selected" : ""}`} key={plan.code}>{plan.highlighted && <span className="plan-ribbon">Più scelto</span>}<p className="plan-audience">{plan.audience}</p><h2>{plan.name}</h2><p className="plan-description">{plan.description}</p><div className="plan-price"><strong>{currency(plan.monthlyPriceCents / 100)}</strong><span>/ mese</span></div><div className="setup-price"><span>Installazione</span><strong>{currency(plan.setupPriceCents / 100)}</strong></div><p className="plan-capacity">{plan.capacity}</p><ul>{plan.features.map((feature) => <li key={feature}><CheckCircle2 size={15} />{feature}</li>)}</ul><div className="plan-limits"><span>{plan.limits.monthlyEmails.toLocaleString("it-IT")} email/mese</span><span>{plan.limits.users ?? "Illimitati"} {plan.limits.users === 1 ? "utente" : "utenti"}</span></div><button className={plan.highlighted ? "primary-button" : "secondary-button"} disabled={paypalActive || planAction !== null} onClick={() => void selectPlan(plan)}><CircleDollarSign size={16} />{paypalActive ? selected ? "Piano attivo" : "Gestisci su PayPal" : planAction === plan.code ? "Apertura PayPal..." : selected ? "Completa su PayPal" : "Prova 14 giorni con PayPal"}</button></article>; })}</section>
        ...
        {!loading && view === "customers" && <div className="view-content reveal"><PageIntro title="Portafoglio clienti" text="Clienti persistenti e isolati per la tua azienda." action={() => setCustomerDialogOpen(true)} actionLabel="Nuovo cliente" />{customers.length ? <section className="data-grid">{customers.map((customer) => <article className="customer-row" key={customer.id}><div className="avatar">{customer.name.split(" ").map((part) => part[0]).join("")}</div><div><strong>{customer.name}</strong><p>{customer.company}</p><small>{customer.email}</small></div><span className={`badge ${customer.status}`}>{customer.status}</span><div className="score"><strong>{customer.satisfaction}%</strong><small>Soddisfazione</small></div></article>)}</section> : <EmptyState title="Nessun cliente" text="Registra il primo cliente per creare e inviare un preventivo." />}</div>}
        ...
        {!loading && view === "quotes" && <div className="view-content reveal"><PageIntro title="Preventivi e offerte" text="PDF salvati in PostgreSQL, scaricabili e inviabili via email con copia alla tua azienda." action={() => customers.length ? setQuoteDialogOpen(true) : setCustomerDialogOpen(true)} actionLabel={customers.length ? "Nuovo preventivo" : "Prima crea un cliente"} />{quotes.length ? <section className="panel"><QuoteTable quotes={quotes} onDownload={downloadQuote} onSend={sendQuote} busyId={quoteAction} /></section> : <EmptyState title="Nessun preventivo" text="Crea il primo preventivo: il PDF verrà generato e salvato automaticamente." />}</div>}
        {!loading && view === "team" && <div className="view-content reveal"><PageIntro title="Personale e presenze" text="Disponibilità e ore registrate nella settimana corrente." /><section className="team-grid">{employees.map((employee) => <article className="team-member" key={employee.id}><div className="member-top"><div className="avatar large">{employee.name.split(" ").map((part) => part[0]).join("")}</div><span className={`presence ${employee.status}`}>{employee.status}</span></div><h3>{employee.name}</h3><p>{employee.role}</p><small>{employee.department}</small><div className="hours"><Clock3 size={17} /><strong>{employee.weeklyHours} h</strong><span>questa settimana</span></div></article>)}</section></div>}
...
function AuthScreen({ onAuthenticated }: { onAuthenticated: (token: string) => void }) {
  ...
  return <main className="auth-shell"><section className="auth-brand"><span className="brand-mark">AO</span><p>AI Office Manager</p><h1>Il centro operativo della tua azienda.</h1><div className="auth-proof"><CheckCircle2 size={18} /><span>Dati isolati per azienda in PostgreSQL</span></div><div className="auth-proof"><CheckCircle2 size={18} /><span>Preventivi PDF salvati e inviati via email</span></div><div className="auth-proof"><CheckCircle2 size={18} /><span>Email e automazioni in tempo reale</span></div></section><section className="auth-panel"><div className="auth-card"><p className="eyebrow">{mode === "register" ? "Configura il workspace" : "Bentornato"}</p><h2>{mode === "register" ? "Registra la tua azienda" : "Accedi ad AI Office"}</h2><p className="auth-copy">{mode === "register" ? "Crea il proprietario e lo spazio dati aziendale." : "Usa le credenziali del tuo account."}</p>{error && <div className="auth-error">{error}</div>}<form onSubmit={submit}>{mode === "register" && <><label>Nome e cognome<input name="name" required autoComplete="name" /></label><label>Nome azienda<input name="companyName" required /></label><label>Email aziendale<input name="companyEmail" type="email" required /></label><div className="form-row"><label>Partita IVA<input name="vatNumber" /></label><label>Città<input name="city" /></label></div></>}<label>Email di accesso<input name="email" type="email" required autoComplete="email" /></label><label>Password<input name="password" type="password" minLength={8} required autoComplete={mode === "register" ? "new-password" : "current-password"} /></label><button className="primary-button auth-submit" disabled={pending} type="submit">{pending ? "Attendere..." : mode === "register" ? "Crea azienda" : "Accedi"}</button></form><button className="auth-switch" onClick={() => { setMode(mode === "register" ? "login" : "register"); setError(""); }}>{mode === "register" ? "Hai già un account? Accedi" : "Nuova azienda? Registrati"}</button></div></section></main>;
}
