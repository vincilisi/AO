...
async function processMessage(mailbox: { email: string }, source: Buffer) {
  ...
  if (!response.ok) throw new Error(`Elaborazione email fallita (${response.status}): ${await response.text()}`);
}
...
async function pollMailbox(mailbox: {
  ...
  try {
    ...
    try {
      ...
      const uids = unread ? unread.slice(-MAX_MESSAGES_PER_MAILBOX) : [];
...
export async function pollEnabledMailboxes() {
  ...
  const results: Array<{ email: string; imported: number; error?: string }> = [];
  for (const mailbox of mailboxes) {
    try {
      results.push({ email: mailbox.email, imported: await pollMailbox(mailbox) });
    } catch (error) {
      results.push({ email: mailbox.email, imported: 0, error: error instanceof Error ? error.message : "Errore IMAP" });
    }
  }
  return { mailboxes: results.length, imported: results.reduce((total, result) => total + result.imported, 0), results };
}